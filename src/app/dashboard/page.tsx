          ...
          return (
            <div key={g.label}>
              ...
              <div className="h-2 overflow-hidden rounded-full bg-black/8">
                <div className="h-2 rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: g.color }} />
              </div>
            </div>
          );
        })}
        ...
        {rubricScores.map((item) => (
          <div key={item.label}>
            ...
            <div className="h-2 rounded-full bg-black/10">
              <div
                className="h-2 rounded-full bg-[linear-gradient(90deg,#fb8500,#ffb703)]"
  ...
  return (
    <article className="rounded-2xl border border-black/10 bg-[linear-gradient(180deg,#ffffff_0%,#fff8ec_100%)] p-5 shadow-[0_12px_30px_rgba(0,0,0,0.08)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_18px_38px_rgba(0,0,0,0.12)]">
      ...
      <div className="mt-5 grid grid-cols-[130px_1fr] items-center gap-4">
        <div
          className="mx-auto h-28 w-28 rounded-full border border-black/10"
          style={{
        ...
        <div className="space-y-2 text-sm">
          {pauseDistribution.map((bucket, idx) => {
            ...
            return (
              <div key={bucket.bucket} className="flex items-center justify-between text-[color:var(--ink-soft)]">
                <span className="inline-flex items-center gap-2">
